...
def extract(files, args, model, mode=''):
    ...
    segments, ans, midi_ids, not_midis = model.prepare_data(files, args.task, int(args.max_len))
    ...
    # 空のMIDIファイルをnpyで保存
    np.save("not_midis.npy", not_midis)
    print(f'not_midis shape: {not_midis.shape}')
